...
struct PrefetcherPimpl
{
	...
	void* VideoCache;
	void* running_workers;
	void* worker_exception_mutex;
	void* worker_exception;
	bool worker_exception_present;
	void* EnvTlsMainThread;
	void* EnvI;
};
...
private:
	...
	int __stdcall SchedulePrefetch(int current_n, int prefetch_start, IScriptEnvironment* env);
	Prefetcher(const PClip& _child, int _nThreads, IScriptEnvironment *env);

public:
	~Prefetcher();
	virtual size_t NumPrefetchThreads() const;
	...
	void Destroy();
	PrefetcherPimpl* GetPImpl() { return _pimpl; }
