namespace IZEncoder.Common.ASSParser
{
    ...
    public struct Color : IEquatable<Color>
    {
        ...
        public byte Alpha
        {
            get => (byte) ~Transparency;
            set => Transparency = (byte) ~value;
        }
        ...
        /// </summary>
        /// <returns>
        ///     The string form of the <see cref="Color" /> in the ass file, which is a <see cref="string" /> started with "
        ///     &amp;H" and followed an 8-digit hex number.
        /// </returns>
        ...
        public static Color FromArgb(byte alpha, byte red, byte green, byte blue)
        {
            unchecked
            {
                return new Color
                {
                    data = ((uint) ~alpha << tOffset) | ((uint) red << rOffset) | ((uint) green << gOffset) |
                           ((uint) blue << bOffset)
                };
            }
        }
