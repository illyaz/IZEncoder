...
namespace IZEncoder.Common.MediaInfo
{
    using System;
    using System.IO;
    ...
    public class MediaInfo : IDisposable
    {
        ...
        {
            var paths = EnvironmentHelper.GetPaths();
            paths.Insert(0, pathToDll);
            pathToDll = Path.GetDirectoryName(DependencySearcher.Search(MediaInfoFileName, paths));
            _module = NativeMethods.LoadLibraryEx(Path.Combine(pathToDll, MediaInfoFileName), IntPtr.Zero,
            ...
            _mustUseAnsi = Environment.OSVersion.ToString().IndexOf("Windows", StringComparison.OrdinalIgnoreCase) ==
                           -1;
        }
        ...
        public IntPtr OpenBufferInit(long fileSize, long fileOffset)
        {
            return _handle == IntPtr.Zero
                ? IntPtr.Zero
                : NativeMethods.MediaInfo_Open_Buffer_Init(_handle, fileSize, fileOffset);
        }
        ...
        public IntPtr OpenBufferContinue(IntPtr buffer, IntPtr bufferSize)
        {
            return _handle == IntPtr.Zero
                ? IntPtr.Zero
                : NativeMethods.MediaInfo_Open_Buffer_Continue(_handle, buffer, bufferSize);
        }
        ...
        {
            if (_handle == IntPtr.Zero) return "Unable to load MediaInfo library";
        ...
        public string Get(StreamKind streamKind, int streamNumber, int parameter, InfoKind kindOfInfo)
        {
            ...
            return _mustUseAnsi
                ? Marshal.PtrToStringAnsi(NativeMethods.MediaInfoA_GetI(_handle, (IntPtr) streamKind,
                    (IntPtr) streamNumber, (IntPtr) parameter, (IntPtr) kindOfInfo))
                : Marshal.PtrToStringUni(NativeMethods.MediaInfo_GetI(_handle, (IntPtr) streamKind,
                    (IntPtr) streamNumber, (IntPtr) parameter, (IntPtr) kindOfInfo));
        }
        ...
        public string Option(string option, string value)
        {
            ...
            if (_mustUseAnsi)
            {
                ...
                using (var optionPtr = GlobalMemory.StringToGlobalAnsi(option))
                {
                    ...
                    {
                        result = Marshal.PtrToStringAnsi(NativeMethods.MediaInfoA_Option(_handle, optionPtr.Handle,
                            valuePtr.Handle));
                    }
                }
        ...
        public int CountGet(StreamKind streamKind, int streamNumber)
        {
            return _handle == IntPtr.Zero
                ? 0
                : (int) NativeMethods.MediaInfo_Count_Get(_handle, (IntPtr) streamKind, (IntPtr) streamNumber);
        }
    ...
    public class MediaInfoList : IDisposable
    {
        ...
        public string Get(int filePos, StreamKind streamKind, int streamNumber, string parameter, InfoKind kindOfInfo,
            InfoKind kindOfSearch)
        {
            return Marshal.PtrToStringUni(NativeMethods.MediaInfoList_Get(_handle, (IntPtr) filePos,
                (IntPtr) streamKind, (IntPtr) streamNumber, parameter, (IntPtr) kindOfInfo, (IntPtr) kindOfSearch));
        }
        ...
        {
            return Marshal.PtrToStringUni(NativeMethods.MediaInfoList_GetI(_handle, (IntPtr) filePos,
        ...
        {
            return (int) NativeMethods.MediaInfoList_Count_Get(_handle, (IntPtr) filePos, (IntPtr) streamKind,
                (IntPtr) streamNumber);
        }
