...
namespace IZEncoder.Common.MediaInfo.Builder
{
    ...
    internal class AudioStreamBuilder : LanguageMediaStreamBuilder<AudioStream>
    {
        ...
        public override AudioStream Build()
        {
            ...
            result.Duration =
                TimeSpan.FromMilliseconds(Get<double>("Duration", double.TryParse, x => ExtractInfo(x, 0)));
        ...
        private string GetFullCodecName()
        {
            ...
            {
                if (!string.IsNullOrEmpty(strCodecVer)) strCodec = (strCodec + " " + strCodecVer).Trim();
            }

            var formatName = Get("Format_Profile").Split('/')[0].Trim();
            if (formatName.IndexOf("ATMOS", StringComparison.OrdinalIgnoreCase) >= 0) return formatName;
        ...
        private static readonly Dictionary<string, AudioCodec> CodecIds =
            new Dictionary<string, AudioCodec>(StringComparer.OrdinalIgnoreCase)
            {
        ...
        private static readonly Dictionary<string, AudioCodec> Codecs =
            new Dictionary<string, AudioCodec>(StringComparer.OrdinalIgnoreCase)
            {
