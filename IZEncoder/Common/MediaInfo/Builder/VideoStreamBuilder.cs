...
namespace IZEncoder.Common.MediaInfo.Builder
{
    ...
    internal class VideoStreamBuilder : LanguageMediaStreamBuilder<VideoStream>
    {
        ...
        public override VideoStream Build()
        {
            ...
            result.FrameRate = Get<double>("FrameRate_Original", double.TryParse);
            if (Math.Abs(result.FrameRate) < double.Epsilon)
                result.FrameRate = Get<double>("FrameRate", double.TryParse);
            result.IsVfr = Math.Abs(result.FrameRate) < double.Epsilon ||
                           Get("FrameRate_Mode").Equals("vfr", StringComparison.OrdinalIgnoreCase);
            ...
            if (Math.Abs(result.Bitrate) < 1E-7) result.Bitrate = Get<double>("BitRate_Maximum", double.TryParse);
            ...
            if (result.Codec == VideoCodec.Undefined) result.Codec = Get<VideoCodec>("Codec", TryGetCodec);
        ...
        private string GetFullCodecName()
        {
            ...
            else
            {
                if (!string.IsNullOrEmpty(strCodecVer))
                {
                    ...
                    if (strCodecProf != "MAIN@MAIN") strCodec = (strCodec + " " + strCodecProf).Trim();
                }
            }
