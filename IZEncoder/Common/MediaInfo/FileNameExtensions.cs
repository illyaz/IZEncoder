namespace IZEncoder.Common.MediaInfo
{
    ...
    {
        private static readonly Regex TsBufferMatch =
            new Regex(@"(live\d+-\d+\.ts(\.tsbuffer(\d+\.ts)?)?)$", RegexOptions.Compiled);
        ...
        public static bool IsNetworkVideo(this string path)
        {
            ...
            return path.StartsWith("rtsp:", StringComparison.OrdinalIgnoreCase) ||
                   ...
                   path.EndsWith(".ymvp", StringComparison.OrdinalIgnoreCase) ||
        ...
        public static bool IsVideo(string path)
        {
            ...
            if (path.Contains(Path.DirectorySeparatorChar) || path.Contains(Path.AltDirectorySeparatorChar) ||
                !path.HasExtension()) return false;
        ...
        {
            if (string.IsNullOrEmpty(path) || path.Contains(Path.DirectorySeparatorChar) ||
                path.Contains(Path.AltDirectorySeparatorChar)) return false;
